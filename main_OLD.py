...
def run_sentiment_analysis(customer_statements: List[str], domain: str) -> List[SentimentResult]:
    """
    Run sentiment analysis on customer statements in manageable batches.

    Args:
        customer_statements (List[str]): The list of customer statements.
        domain (str): The domain of the analysis.

    Returns:
        List[SentimentResult]: A combined list of sentiment analysis results for all customer statements.
    """
    logger.info("Running sentiment analysis in batches...")
    max_statements_per_batch = 50  # This will help avoid hitting the token limit.
    sentiment_results = []

    # Split the statements into batches
    for i in range(0, len(customer_statements), max_statements_per_batch):
        batch = customer_statements[i:i + max_statements_per_batch]
        batch_text = "\n".join(batch)

        try:
            # Run the sentiment chain on the current batch
            sentiment_sequence = create_sentiment_chain()
            raw_output = sentiment_sequence.invoke({
                "statements": batch_text,
                "domain": domain,
            })
            if raw_output and raw_output.results:
                sentiment_results.extend(raw_output.results)

        except OutputParserException as e:
            logger.error(f"Failed to parse sentiment results for batch starting at index {i}: {e}")
        except Exception as e:
            logger.error(f"An unexpected error occurred during sentiment analysis for batch starting at index {i}: {e}")

    # Log the number of results obtained
    logger.info(f"Total statements analyzed: {len(sentiment_results)}")
